...
public class RetrofitClientInstanceUrl {
    ...
    public static Retrofit getRetrofitInstanceDefaultTokenNewUrl(String url) {
        ...
        if (f6retrofit2 == null) {
            f6retrofit2 = new Retrofit.Builder().baseUrl(url).addConverterFactory(ScalarsConverterFactory.create()).client(client).addConverterFactory(GsonConverterFactory.create(new GsonBuilder().setLenient().create())).build();
        }
        return f6retrofit2;
    }
}
